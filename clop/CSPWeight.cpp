...
// Constructor
/////////////////////////////////////////////////////////////////////////////
CSPWeight::CSPWeight(const CRegression &reg, int ReplicationThreshold, int nMCMC)
    : vResult(reg.GetPF().GetDimensions()), reg(reg), ReplicationThreshold(ReplicationThreshold), nMCMC(nMCMC) {}
...
const double *CSPWeight::NextSample(int i) {
	...
	//
	if (ReplicationThreshold && i > 0 && reg.GetReplications(i - 1) < (1 + i / ReplicationThreshold))
